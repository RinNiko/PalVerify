...
Require-Text $launcherSource 'launcher_version = "1.0.31"' `
    "launcher must ship the coordinator-clock heartbeat fix"
...
Require-NoText $palHudSource 'SendSystemToPlayerChat' `
    "bundled PalHud must not use the PalDefender-sensitive chat transport"
Require-Text $palHudSource 'GetPlayerName' `
    "bundled PalHud must resolve runtime HUD state through player names"
Require-NoText $palHudSource 'BP_GetUniqueId' `
    "bundled PalHud must not call the crashing player UniqueId path"
Require-NoText $palHudSource 'GetPlayerUniqueIdToString' `
    "bundled PalHud must not convert player UniqueId during HUD delivery"
Require-Text $palHudSource 'K2_OnLogout' `
    "bundled PalHud must release cached controllers before logout"
Require-Text $palHudSource 'UnPossess' `
    "bundled PalHud must release cached controllers before unpossess"
Require-NoText $palHudSource 'render_local_preview' `
    "bundled PalHud must never render local preview data"
Require-NoText $palHudSource 'runtime.local_preview' `
...
Require-Text $launcherSource 'palverify_version = "1.0.15"' `
    "launcher must bundle PalVerify verifier version 1.0.15"
...
Require-Text $releaseManifest '"launcherVersion": "1.0.31"' `
    "stable manifest must trigger the coordinator-clock heartbeat fix"
Require-Text $releaseManifest '"minimumLauncherVersion": "1.0.31"' `
    "stable manifest must require the coordinator-clock heartbeat fix"
...
Require-Text $packageInfo '"Version": "1.0.15"' `
    "PalVerify package metadata must expose verifier version 1.0.15"
Require-Text $clientAgentSource 'CLIENT_STARTED protocol=3 version=1.0.15' `
    "client runtime log must expose verifier version 1.0.15"
